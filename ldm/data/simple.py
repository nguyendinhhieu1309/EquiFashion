...
class FolderData(Dataset):
    ...
    def __getitem__(self, idx):
        dataset={}
        item = self.data[idx]
        image_filename = item['gt']
        prompt = item['caption']

        image_path = str(self.root_dir) + image_filename
        image = cv2.imread(image_path)  # Read image using OpenCV

        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        input_image = HWC3(image)
        # Resize the image to 512 resolution
        res_image = resize_image(input_image, 512) # 768*1024->512*704

        target = (res_image.astype(np.float32) / 127.5) - 1.0
