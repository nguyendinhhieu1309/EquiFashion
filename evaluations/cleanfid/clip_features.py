...
class CLIP_fx():
    def __init__(self, name="ViT-B/32", device="cuda"):
        ...
        checkpoint = torch.load(f"/data/lh/docker/models/my_train/my_clip_checkpoints_44epochs.ckpt",map_location='cuda')
        pretrained_dict = {key.replace("model.", ""): value for key, value in checkpoint["state_dict"].items()}
        self.model.load_state_dict(pretrained_dict)  # ['model_state_dict'])
